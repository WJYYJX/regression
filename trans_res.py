...
class Attention2(nn.Module):
    ...
    def forward(self, x):
        ...
        k55 = k5[:,None,:,:]
        k66 = k6[:,None,:,:]
        kk = torch.cat((k11, k22, k33, k44, k55, k66), dim=1 )

        #kk = kk - torch.mean(kk, -1, keepdim=True)
        ...
        # k55 = k5[:,:,:,np.newaxis]
        # k66 = k6[:,:,:,np.newaxis]
        # kk=np.concatenate((k1, k22, k33, k44, k55, k66), axis=3)
        # kk1 = kk.transpose(0, 3, 1, 2)

        # kk = self.kc(x - torch.mean(x, -1, keepdim=True))  # 引导
        k = rearrange(kk, 'b n1 n2 (h d) -> b  n1 h n2  d', h=self.heads)
...
class trans_conv(nn.Module):
    def __init__(self, inplane, outplane, kernelsize=1, padding=0, stride=1, size=(4, 4)):
        ...
        patch_width = 1
        self.resh = Rearrange('b (c k) (h p1) (w p2) -> b  (k p1 p2) (h w) c', k=6, p1=patch_height,
                              p2=patch_width)  # 合并维度  #手动切patch1*1
        self.resh2 = Rearrange('b (k p1 p2) (h w)  c -> b (c k) (h p1) (w p2)', k=6, p1=patch_height, p2=patch_width,
                               h=math.ceil(size[0] / patch_height), w=math.ceil(size[1] / patch_width))  # 还原
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=self.heads),
                      qkv)  # qkv的shape [b,num_heads,patch_size,dim//num_heads]
...
class ResNet18(nn.Module):
    def __init__(self, model, num_classes=1000):
        ...
        ss = 2
        self.mask = mack_conv(6, ss ** 2).to(device)
        # self.mask=None
